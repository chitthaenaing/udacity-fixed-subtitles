...
어떤 작업을 수행해야 할 데이터에 관한 것이다.
우리가 활동에 종사하는 위치이뿐만 아니라
...
정보를 읽을 때 우리는 동일한 키를 사용하고 있습니다.
그냥 토스트처럼, 우리는 예측을 얻을
현재 목록 항목에서 호출하여 데이터 get
지정된 위치의 어댑터 아이템. 한때
우리는 우리가 방금 활동을 시작하고 의도를 추가했습니다.




14 - DetailActivity 표시 내용
======================================
...
유후. 우리의 목록 항목을 클릭하면 작동합니다.
 ...
 우리의 응용 프로그램은 훌륭합니다.
 응.
 하지만 설정 메뉴를 클릭하면 아무것도하지 않습니다.도단 소리를내는 도단 소리를 세운다. #sadtrombone.
 MM은 다시 그것을하지 마십시오. 내가 저장하세요
...
그럼 설정 UI 와이어 프레임보고 갑시다. 오버플로 메뉴에 대한 자세한 활동 속에서 설정 메뉴 항목이 있습니다.
만약 설정 메뉴 항목을 누르면 해당 설정 작업을 연다.당신이 장소를 탭하면 그것은 당신이 당신 안에 들어갈 수있는 대화 상자를 팝업 표시
...
이상에서 마법사가 무엇을하고 있는지를 더 잘 이해할 수 있다고 생각합니다.
설정이 완료되면 이러한 항목을 해제 확인하십시오.
...
Mainactivity.java 온 옵션 항목 선택한 방법으로.
설정 메뉴 항목이 선택되면 우리
...
선택되어있는 우리는 SettingsActivity 새로운 의도를 시작합니다.
...
다음 몇 섹션에서 우리의 두 가지 설정을 추가하게 될 것이다. 우리는 우리의 첫 번째 설정을 추가하는 방법을 단계별 해요
...
이들은 우리가 선언 된 문자열입니다. 이것은 취향의 레이블 이름입니다.
...
지금,이 값입니다. 안드로이드에서 이러한 변경을 할 수 있습니다. 당신이 필요로하는 코드 라인을위한 강사 노트를 참조하십시오.
...
당신이 계속 확인란을 선택하고 변경했습니다.





26 - 이용 SharedPreferences 솔루션
===================================
...
이미 실행되지 않았다는 것을. 이미 실행하고있는 경우, 그것을 죽인다.
그리고 응용 프로그램을 여는 데 누르고
...
활동이 성공적으로 해결 여부, 우리는 유일하게 활동을 시작한다.
...
에 관심이 있습니다. 우리는 이전부터 우리의 봉투 은유로 돌아 가기한다면 중간
방송의 의도 예, 당신의
...
첫 번째는 Android의 역사에 또 다른 여행을 보자.




41 - Storytime Android 배달 플랫폼
============================================
...
앱 설치 수십억.놀라운 기회에
국가의 수백 Android 사용자에게 응용 프로그램을 배포합니다.
그런데, 안드로이드 초기에 빨리있다
